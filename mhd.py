...
from my_classes import MeshXDMF
...
def mhd_sim(
    ...
    F2 = rho * dot((u - u_n) / k, v) * mesh.dx
    F2 += rho * dot(dot(u_n, nabla_grad(u_n)), v) * mesh.dx
    F2 += inner(sigma(U, p_n), epsilon(v)) * mesh.dx
    ...
    a4 = form(rho * dot(u, v) * mesh.dx)
    L4 = form(rho * dot(u_, v) * mesh.dx - k * dot(nabla_grad(p_ - p_n), v) * mesh.dx)
...
def mhd_sim_with_salome_mesh(
    ...
    mesh_folder = "mesh/"
    my_mesh = MeshXDMF(
        cell_file=mesh_folder + "mesh_domains_alt.xdmf",
        facet_file=mesh_folder + "mesh_boundaries_alt.xdmf",
        subdomains=[],
    )
    ...
    dt = dt  # Time step size
    num_steps = int(T / dt)
    k = Constant(my_mesh.mesh, PETSc.ScalarType(dt))

    # Define elements and function spaces
    velocity_ele = VectorElement("CG", my_mesh.mesh.ufl_cell(), 2)
    electric_potential_ele = FiniteElement("CG", my_mesh.mesh.ufl_cell(), 1)
    pressure_ele = FiniteElement("CG", my_mesh.mesh.ufl_cell(), 1)

    V = FunctionSpace(my_mesh.mesh, velocity_ele)
    Q = FunctionSpace(my_mesh.mesh, electric_potential_ele)
    Q2 = FunctionSpace(my_mesh.mesh, pressure_ele)

    # Define boundary conditions
    fdim = my_mesh.mesh.topology.dim - 1
    bc_fully_conductive = dirichletbc(
        PETSc.ScalarType(0), locate_dofs_topological(Q, fdim, ft.find(id_hartmann_walls)), Q
    )

    bc_noslip_hw = dirichletbc(PETSc.ScalarType((0, 0, 0)), locate_dofs_topological(V, fdim, ft.find(id_hartmann_walls)), V)
    bc_noslip_w = dirichletbc(PETSc.ScalarType((0, 0, 0)), locate_dofs_topological(V, fdim, ft.find(id_walls)), V)

    bc_inflow = dirichletbc(PETSc.ScalarType((10, 0, 0)), locate_dofs_topological(V, fdim, ft.find(id_inlet)), V)
    bc_outflow = dirichletbc(PETSc.ScalarType(0), locate_dofs_topological(Q2, fdim, ft.find(id_outlet)), Q2)
    ...
    bcu = [bc_noslip_hw, bc_noslip_w, bc_inflow]
    bcp = [bc_outflow]
    ...
    # Define constant parameters
    Ha = Constant(my_mesh.mesh, (PETSc.ScalarType(Ha_no)))
    N = Ha**2
    mu = Constant(my_mesh.mesh, PETSc.ScalarType(1))  # Dynamic viscosity
    rho = Constant(my_mesh.mesh, PETSc.ScalarType(1))  # Density
    B = Constant(
        my_mesh.mesh, (PETSc.ScalarType(0), PETSc.ScalarType(-1), PETSc.ScalarType(0))
    )
    ...
    # dx = my_mesh.mesh.dx(metadata={'quadrature_degree': 3})

    # Step 1: Evaluate the electrical potential
    F1 = (
        inner(grad(phi), grad(q)) * my_mesh.dx
        - inner(dot(curl(u_n), B) + dot(u_n, curl(B)), q) * my_mesh.dx
    )
    a1 = form(lhs(F1))
    L1 = form(rhs(F1))
    if conductive is True:
    ...
    solver1 = PETSc.KSP().create(my_mesh.mesh.comm)
    solver1.setOperators(A1)
    solver1.setTolerances(rtol=1e-12)
    solver1.setTolerances(atol=1e-08)
    solver1.setType(PETSc.KSP.Type.BCGS)
    ...
    # Step 2: Tentative velocity step
    F2 = rho * dot((u - u_n) / k, v) * my_mesh.dx
    F2 += rho * dot(dot(u_n, nabla_grad(u_n)), v) * my_mesh.dx
    F2 += inner(sigma(U, p_n), epsilon(v)) * my_mesh.dx
    F2 += dot(p_n * n, v) * ds - dot(mu * nabla_grad(U) * n, v) * ds
    F2 += N * (
        inner(cross(B, grad(phi_)), v) * my_mesh.dx
        + inner(u_n * dot(B, B), v) * my_mesh.dx
        - inner(B * dot(B, u_n), v) * my_mesh.dx
    )
    a2 = form(lhs(F2))
    L2 = form(rhs(F2))
    A2 = assemble_matrix(a2, bcs=bcu)
    ...
    solver2 = PETSc.KSP().create(my_mesh.mesh.comm)
    solver2.setOperators(A2)
    solver2.setType(PETSc.KSP.Type.BCGS)
    solver2.setTolerances(rtol=1e-08)
    solver2.setTolerances(atol=1e-08)
    ...
    # Step 3: Pressure corrrection step
    a3 = form(dot(nabla_grad(p), nabla_grad(q2)) * my_mesh.dx)
    L3 = form(
        dot(nabla_grad(p_n), nabla_grad(q2)) * my_mesh.dx
        - (rho / k) * div(u_) * q2 * my_mesh.dx
    )
    A3 = assemble_matrix(a3, bcs=bcp)
    A3.assemble()
    b3 = create_vector(L3)

    solver3 = PETSc.KSP().create(my_mesh.mesh.comm)
    solver3.setOperators(A3)
    solver3.setType(PETSc.KSP.Type.MINRES)
    solver3.setTolerances(rtol=1e-08)
    solver3.setTolerances(atol=1e-08)
    ...
    # Step 4: Velocity correction step
    a4 = form(rho * dot(u, v) * my_mesh.dx)
    L4 = form(rho * dot(u_, v) * my_mesh.dx - k * dot(nabla_grad(p_ - p_n), v) * my_mesh.dx)
    A4 = assemble_matrix(a4)
    A4.assemble()
    b4 = create_vector(L4)

    solver4 = PETSc.KSP().create(my_mesh.mesh.comm)
    solver4.setOperators(A4)
    solver4.setType(PETSc.KSP.Type.CG)
    solver4.setTolerances(rtol=1e-08)
    solver4.setTolerances(atol=1e-08)
    ...
    u_xdmf = XDMFFile(my_mesh.mesh.comm, results_foldername + "u.xdmf", "w")
    u_xdmf.write_mesh(my_mesh.mesh)
    p_xdmf = XDMFFile(my_mesh.mesh.comm, results_foldername + "p.xdmf", "w")
    p_xdmf.write_mesh(my_mesh.mesh)
    phi_xdmf = XDMFFile(my_mesh.mesh.comm, results_foldername + "phi.xdmf", "w")
    phi_xdmf.write_mesh(my_mesh.mesh)
...
if __name__ == "__main__":
    ...
    mhd_sim_with_salome_mesh(
        Ha_no=10,
        conductive=True,
        results_foldername="Results/with_salome_mesh/",
        total_time=5e-01,
        dt=5e-03,
        export_mode=1,
    )
