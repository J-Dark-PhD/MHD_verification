...
def mhd_sim(
    ...
    top_bottom_wall_facets = locate_entities_boundary(
        mesh, mesh.topology.dim - 1, insulated_walls
    )
    top_bottom_dofs = locate_dofs_topological(
        V2.sub(2), mesh.topology.dim - 1, top_bottom_wall_facets
    )
    bc_insulated_walls = dirichletbc(PETSc.ScalarType(0), top_bottom_dofs, V2.sub(2))
    hartmann_walls_facets = locate_entities_boundary(
        mesh, mesh.topology.dim - 1, hartmann_walls
    )
    hartmann_walls_dofs = locate_dofs_topological(
        V2.sub(1), mesh.topology.dim - 1, hartmann_walls_facets
    )
    bc_hartmann_walls_insulated = dirichletbc(
        PETSc.ScalarType(0), hartmann_walls_dofs, V2.sub(1)
    )
    ...
    solver5.setTolerances(rtol=1e-08)
    solver5.setTolerances(atol=1e-08)
    pc5 = solver5.getPC()
    ...
    solver6.setTolerances(rtol=1e-08)
    solver6.setTolerances(atol=1e-08)
    pc6 = solver6.getPC()
    ...
    for i in range(num_steps):
        ...
        set_bc(b4, bcu)
        print("solving tentitive velocity")
        ...
        # F_xdmf.write_function(F_lorentz_W t)
...
if __name__ == "__main__":
    ...
    mhd_sim(
        Ha_no=100, conductive=False, total_time=5e-02, dt=1e-04, Nx=20, Ny=30, Nz=30
    )
